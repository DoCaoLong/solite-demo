(function () {
  ...
  function draw(opts) {
    ...
    btn = document.querySelector(".hc-luckywheel-btn");
    ...
    for (var i = 0; i < num; i++) {
      ctx.save();
      ctx.beginPath();
      ctx.translate(230, 230); // Center Point
      ctx.moveTo(0, 0);
  ...
  function events() {
    bind(btn, "click", function () {
      ...
      fnGetPrize(function (data) {
        ...
        deg = deg || 0;
        deg =
          deg + (360 - (deg % 360)) + (360 * 10 - data[0] * (360 / num)) + 20;
        runRotate(deg);
      });
  ...
  // check data gift && call func fnGotBack
  function eGot() {
  ...
  // checks if the module system is present (AMD) and defines the module "HellCat-Luckywheel" if available.
